"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : new P(function (resolve) { resolve(result.value); }).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
const database_1 = __importDefault(require("../db/database"));
class JuegosController {
    listarJuegos(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            const juegos = yield database_1.default.query('SELECT * FROM Juegos');
            res.json(juegos);
        });
    }
    listarJuego(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            const { id } = req.params;
            const juego = yield database_1.default.query('SELECT * FROM Juegos WHERE idJuego = ?', [id]);
            // Si el juego existe
            if (juego.length > 0)
                return res.json(juego[0]);
            res.status(404).json({ mensaje: 'El juego no existe' });
        });
    }
    crearJuego(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            yield database_1.default.query('INSERT INTO Juegos set ?', [req.body]);
            res.json({ mensaje: 'Juego guardado' });
        });
    }
    actualizarJuego(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            const { id } = req.params;
            yield database_1.default.query('UPDATE Juegos set ? WHERE idJuego = ?', [req.body, id]);
            res.json({ mensaje: 'Juego actualizado' });
        });
    }
    eliminarJuego(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            const { id } = req.params;
            yield database_1.default.query('DELETE FROM Juegos WHERE idJuego = ?', [id]);
            res.json({ mensaje: 'Juego eliminado' });
        });
    }
}
exports.juegosController = new JuegosController();
